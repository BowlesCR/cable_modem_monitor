***REMOVED*** Security Linting Setup for VS Code

This guide helps you set up local security linting to catch CodeQL-like issues before pushing code.

***REMOVED******REMOVED*** Quick Start

***REMOVED******REMOVED******REMOVED*** 1. Install Security Tools

```bash
***REMOVED*** Install Python security linters
pip install -r requirements-security.txt

***REMOVED*** Or install individually
pip install bandit semgrep safety flake8-security
```

***REMOVED******REMOVED******REMOVED*** 2. Install VS Code Extensions
...
***REMOVED******REMOVED******REMOVED*** 3. Run Security Scans

```bash
***REMOVED*** Run all security scans
./scripts/security-scan.sh

***REMOVED*** Or run individually
bandit -c .bandit -r custom_components/
semgrep --config=.semgrep.yml custom_components/
safety check
```

***REMOVED******REMOVED*** Tool Details

***REMOVED******REMOVED******REMOVED*** 🔍 Bandit (Python Security Linter)
...
**Run manually:**
```bash
***REMOVED*** Scan all code
bandit -r custom_components/

***REMOVED*** With configuration
bandit -c .bandit -r custom_components/
...
***REMOVED******REMOVED******REMOVED*** 🎯 Semgrep (Multi-language Security Scanner)
...
**Run manually:**
```bash
***REMOVED*** Scan with custom rules
semgrep --config=.semgrep.yml custom_components/

***REMOVED*** Scan with community rules
semgrep --config=auto custom_components/

***REMOVED*** Output formats
...
***REMOVED******REMOVED******REMOVED*** 🛡️ Safety (Dependency Vulnerability Scanner)
...
**Run manually:**
```bash
***REMOVED*** Scan requirements.txt
safety check

***REMOVED*** Scan specific file
...
***REMOVED******REMOVED******REMOVED*** 🔬 CodeQL (Official GitHub Scanner)

**Setup for local use:**

1. **Install CodeQL CLI:**
```bash
***REMOVED*** Download from https://github.com/github/codeql-cli-binaries/releases
...
```bash
***REMOVED*** Create database for Python
codeql database create codeql-db --language=python

***REMOVED*** Or specify source root
codeql database create codeql-db --language=python --source-root=custom_components/
```

3. **Run queries:**
```bash
***REMOVED*** Run security queries
...
***REMOVED******REMOVED*** Common Security Issues & Fixes

***REMOVED******REMOVED******REMOVED*** 1. SSL Verification Disabled
...
***REMOVED*** Or make it configurable
requests.get(url, verify=self.verify_ssl)
```

***REMOVED******REMOVED******REMOVED*** 2. Command Injection
...
***REMOVED******REMOVED******REMOVED*** 3. Unvalidated Redirects
...
if response.status_code in [301, 302]:
    ...
    if is_safe_redirect(original_url, redirect_url):
        ***REMOVED*** Follow redirect
```

***REMOVED******REMOVED******REMOVED*** 4. Logging Sensitive Data
...
***REMOVED******REMOVED******REMOVED*** 5. Broad Exception Catching
...
***REMOVED******REMOVED******REMOVED*** 6. F-strings in Logging

**Semgrep:** `f-string-in-logging`

```python
***REMOVED*** ❌ Bad (premature evaluation)
_LOGGER.debug(f"Processing {item}")

***REMOVED*** ✅ Good (lazy evaluation)
_LOGGER.debug("Processing %s", item)
```

***REMOVED******REMOVED*** Pre-commit Hook (Recommended)
...
***REMOVED******REMOVED*** CI Integration
...
***REMOVED******REMOVED******REMOVED*** Bandit False Positives

Skip specific checks:
```python
***REMOVED*** Skip specific line
password = get_password()  ***REMOVED*** nosec B105

***REMOVED*** Skip specific check
***REMOVED*** nosec B603
```

***REMOVED******REMOVED******REMOVED*** Semgrep False Positives
...
***REMOVED******REMOVED******REMOVED*** VS Code Not Showing Issues
...
***REMOVED*** Watch mode (auto-scan on file changes)
