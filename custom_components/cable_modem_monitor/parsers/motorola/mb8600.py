...
class MotorolaMB8600HnapParser(ModemParser):
    ...
    # Parser status - Self-tested @BowlesCR in issue #40 (December 2025)
    status = ParserStatus.VERIFIED
    verification_source = "https://github.com/solentlabs/cable_modem_monitor/issues/40"
    # Device metadata
    release_date = "2017"
    docsis_version = "3.1"
    fixtures_path = "tests/parsers/motorola/fixtures/mb8600"
    ...
    # Note: MB8600 returns OFDM channels (modulation="OFDM PLC") in the same
    ...
    def login(self, session, base_url, username, password) -> tuple[bool, str | None]:
        ...
        Some MB8600 firmware variants use JSON-formatted HNAP authentication,
        ...
        _LOGGER.debug("MB8600: Attempting JSON-based HNAP login")
        success: bool
        response: str | None
        success, response = self._json_builder.login(session, base_url, username, password)

        if success:
            _LOGGER.info("MB8600: JSON HNAP login successful")
        ...
        _LOGGER.debug("MB8600: JSON login failed, trying XML/SOAP-based HNAP login")
        ...
        if success:
            _LOGGER.info("MB8600: XML/SOAP HNAP login successful")
        else:
            _LOGGER.warning("MB8600: Both JSON and XML/SOAP HNAP login methods failed")
    ...
    def parse(self, soup: BeautifulSoup, session=None, base_url=None) -> dict:
        ...
        try:
            ...
            _LOGGER.debug("MB8600: JSON HNAP failed (%s), trying XML/SOAP HNAP...", str(json_error))
            ...
            try:
                ...
                _LOGGER.error(
                    "MB8600: Both JSON and XML/SOAP HNAP methods failed. " "JSON error: %s, XML error: %s",
                    str(json_error),
                    str(xml_error),
                    exc_info=True,
                )
    ...
    def _parse_with_json_hnap(self, session, base_url: str) -> dict:
        ...
        if self._json_builder is not None:
            ...
            _LOGGER.debug("MB8600: Reusing JSON builder from login (private_key preserved)")
        else:
            builder = HNAPJsonRequestBuilder(
                endpoint=self.auth_config.hnap_endpoint, namespace=self.auth_config.soap_action_namespace
            )
            _LOGGER.warning("MB8600: No stored JSON builder - creating new one (may lack auth)")
    ...
    def _parse_with_xml_hnap(self, session, base_url: str) -> dict:
        ...
        _LOGGER.debug("MB8600: Attempting XML/SOAP-based HNAP communication")
        ...
        _LOGGER.debug("MB8600: Fetching modem data via XML/SOAP HNAP GetMultipleHNAPs")
        ...
        # Enhanced logging to help diagnose response structure
        _LOGGER.debug(
            "MB8600: XML/SOAP HNAP response received. Top-level keys: %s, response size: %d bytes",
            list(hnap_data.keys()),
            len(json_response),
        )
        ...
        _LOGGER.info(
            "MB8600: Successfully parsed data using XML/SOAP HNAP " "(downstream: %d channels, upstream: %d channels)",
            len(downstream),
            len(upstream),
        )
    ...
    def _extract_software_info(self, hnap_data: dict, system_info: dict) -> None:
        ...
        # StatusSoftwareSfVer contains the firmware version (e.g., "8600-19.3.15")
    ...
    def restart(self, session, base_url) -> bool:
        """Restart the MB8600 modem via HNAP SetStatusSecuritySettings action.
        ...
        try:
            ...
            if self._json_builder is not None:
                ...
                _LOGGER.warning("MB8600: No stored JSON builder for restart - may lack auth")
            ...
            if result == "OK":
                _LOGGER.info("MB8600: Restart command sent successfully")
                return True
            else:
                # Enhanced error diagnostics - log full request/response for debugging
                _LOGGER.error("MB8600: Restart failed with result: %s", result)
                _LOGGER.error(
                    "MB8600: Restart diagnostics - Request: action=SetStatusSecuritySettings, " "params=%s",
                    restart_data,
                )
                _LOGGER.error("MB8600: Restart diagnostics - Full response: %s", response)
                return False

        except ConnectionResetError:
            # Connection reset is expected - modem reboots immediately
            _LOGGER.info("MB8600: Restart sent successfully (connection reset by rebooting modem)")
            return True
        except Exception as e:
            if "Connection aborted" in str(e) or "Connection reset" in str(e):
                _LOGGER.info("MB8600: Restart sent successfully (connection reset by rebooting modem)")
                return True
            _LOGGER.error("MB8600: Error sending restart command: %s", e)
            return False
